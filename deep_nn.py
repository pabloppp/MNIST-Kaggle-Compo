...
def compute_cost_with_regularization(AL, Y, parameters, lambd):
    ...
    for l in range(L):
        regularization_cost = regularization_cost + np.sum(np.square(parameters["W" + str(l + 1)]))
    regularization_cost = (1 / m) * (lambd / 2) * regularization_cost
    cost = compute_cost(AL, Y) + regularization_cost
    return cost
